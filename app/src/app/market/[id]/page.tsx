      ...
      <div className="max-w-4xl mx-auto px-6 py-8">
        ...
        >
          <div className="flex items-start justify-between mb-5 gap-4">
            ...
            <span
              className={`shrink-0 text-[10px] px-3 py-1.5 rounded-full border font-medium tracking-wide ${getStatusBadgeColor(market.status)}`}
            >
          ...
          <div className="mb-7">
            <div className="flex justify-between mb-2.5">
              <div>
                <span className="text-emerald-400 text-3xl font-bold tracking-tight">
                  {yesPercent.toFixed(1)}%
                </span>
                <span className="text-emerald-400/50 text-sm ml-2 font-medium">YES</span>
              </div>
              <div className="text-right">
                <span className="text-rose-400 text-3xl font-bold tracking-tight">
          ...
          <div className="mt-6 pt-5 border-t border-[#1a1a2e]/40 grid grid-cols-2 gap-3 text-[13px]">
            <div>
              ...
              <span className="font-medium">Price {market.targetAbove ? "above" : "below"} {market.targetPrice}</span>
            </div>
            <div>
              <span className="text-zinc-600">Oracle </span>
              <span className="font-mono text-[11px]">{truncateAddress(market.oracleFeed, 6)}</span>
            </div>
            <div>
              <span className="text-zinc-600">Min Bet </span>
              <span>{(market.minBet / 1e9).toFixed(2)} SOL</span>
            </div>
            <div>
              <span className="text-zinc-600">Max Bet </span>
            ...
            <div>
              ...
              <span className="font-mono text-[11px]">{truncateAddress(market.creator, 6)}</span>
            </div>
          </div>
          ...
          {market.status === "resolved" && (
            <div className="mt-5 p-4 rounded-xl bg-violet-500/8 border border-violet-500/15">
              ...
              <p className="text-[13px] text-zinc-500">
                Oracle price: {market.resolvedPrice} • {market.resolvedAt && formatTimestamp(market.resolvedAt)}
              </p>
            </div>
          )}
        </motion.div>
        ...
        >
          <h3 className="text-base font-semibold mb-5">
            Bets <span className="text-zinc-600 font-normal ml-1.5">{bets.length}</span>
          </h3>
          {bets.length === 0 ? (
              ...
              <table className="w-full text-[13px]">
                ...
                <tbody>
                  {bets.map((bet) => (
                    <tr
                      key={bet.publicKey}
                      className="border-b border-[#1a1a2e]/30 hover:bg-white/[0.015] transition"
                    >
                      ...
                      <td className="py-3.5 px-3">
                        <span
                          className={`inline-flex px-2.5 py-0.5 rounded-md text-[10px] font-semibold tracking-wide ${
                            bet.position === "YES"
                      ...
                      <td className="py-3.5 px-3">
                        {bet.claimed ? (
                          <span className="text-violet-400 text-[11px]">Claimed</span>
                        ) : (
                          <span className="text-zinc-600 text-[11px]">Pending</span>
                        )}
                      </td>
                      <td className="py-3.5 px-3 text-[11px] text-zinc-600">
